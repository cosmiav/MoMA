    ...
    <div class="row pb-3 justify-content-between">
      <div class="col-md-6 col-lg-5 align-self-center">
        <div class="aspect-ratio-container">
          <img :src="nearestUpcomingExhibition.image" class="img-fluid aspect-ratio-image rounded-0 border-0" />
        </div>
      </div>
      <div class="col-md-5 col-lg-6 align-self-center">
        <p class="my-3">Upcoming Exhibition</p>
        <h3>{{ nearestUpcomingExhibition.title }}</h3>
        <h5>{{ formatDateMonth(nearestUpcomingExhibition.start_date) }} - {{ formatDateMonth(nearestUpcomingExhibition.end_date) }}</h5>
        <p>{{ nearestUpcomingExhibition.location }}</p>
        <br />
        <router-link :to="{ name: 'ExhibitionDetail', params: { id: nearestUpcomingExhibition.id } }">
          <p>Learn More</p>
        </router-link>
      </div>
    </div>
...
export default {
  setup() {
    const { exhibition, formatDateMonth } = useDB();
    const nearestUpcomingExhibition = ref(null);

    const filterNearestUpcomingExhibition = () => {
      const upcoming = exhibition.value.filter((item) => item.status === 'upcoming');
      if (upcoming.length) {
        nearestUpcomingExhibition.value = upcoming.reduce((nearest, current) => {
